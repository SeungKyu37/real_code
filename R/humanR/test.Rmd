...
```{r}
# 코드 작성
kepler1 %>% select(NAME, NATION) %>%  filter(NATION == "일본")
```

- kepler1 데이터 세트에서 “POSITION”이 메인보컬이고 “나이”가 20살 이하인 맴버를 선별하여 표시하는 코드를 작성하시오.[10]
```{r}
# 코드 작성
kepler1 %>% select(NAME,POSITION,AGE) %>% filter(POSITION == "메인보컬" & AGE <= 20)
```
...
library(ggplot2)
# 코드 작성
ggplot(kepler1,aes(x=AGE))+geom_histogram()+
